...
public class ShaderUtils {
    ...
    private final String kawaseUpBloom = "#version 120\n" +
            ...
            "        vec2(-1.0, -1.0)\n" +
            "    );\n" +
            "    float w[8] = float[8](1.0, 2.0, 1.0, 2.0, 1.0, 2.0, 1.0, 2.0);\n" +
            "    vec4 sum = vec4(0.0);\n" +
            "    for(int i = 0; i < 8; ++i){\n" +
            "        vec2 off = dirs[i] * halfpixel * offset;\n" +
            "        vec4 t = texture2D(inTexture, uv + off);\n" +
            "        t.rgb *= t.a;\n" +
            "        sum += t * w[i];\n" +
            "    }\n" +
            "    vec4 result = sum / 12.0;\n" +
            "    float ca = texture2D(textureToCheck, gl_TexCoord[0].st).a;\n" +
            "    gl_FragColor = vec4(result.rgb / result.a, mix(result.a, result.a * (1.0 - ca), float(check)));\n" +
            "}\n";
    private final String kawaseDownBloom = "#version 120\n" +
            ...
            "    sum *= 4.0;\n" +
            "    vec4 t;\n" +
            "    t = texture2D(inTexture, uv - halfpixel.xy * offset); t.rgb *= t.a; sum += t;\n" +
            "    t = texture2D(inTexture, uv + halfpixel.xy * offset); t.rgb *= t.a; sum += t;\n" +
            "    t = texture2D(inTexture, uv + vec2(halfpixel.x, -halfpixel.y) * offset); t.rgb *= t.a; sum += t;\n" +
            "    t = texture2D(inTexture, uv - vec2(halfpixel.x, -halfpixel.y) * offset); t.rgb *= t.a; sum += t;\n" +
            "    vec4 result = sum / 8.0;\n" +
    ...
    private final String kawaseUp = "#version 120\n" +
            ...
            "    vec4 sum = vec4(0.0);\n" +
            "    vec4 t;\n" +
            "    t = texture2D(inTexture, uv + vec2(-halfpixel.x * 2.0, 0.0) * offset); sum += t;\n" +
            "    t = texture2D(inTexture, uv + vec2(-halfpixel.x, halfpixel.y) * offset); sum += t * 2.0;\n" +
            "    t = texture2D(inTexture, uv + vec2(0.0, halfpixel.y * 2.0) * offset); sum += t;\n" +
            "    t = texture2D(inTexture, uv + vec2(halfpixel.x, halfpixel.y) * offset); sum += t * 2.0;\n" +
            "    t = texture2D(inTexture, uv + vec2(halfpixel.x * 2.0, 0.0) * offset); sum += t;\n" +
            "    t = texture2D(inTexture, uv + vec2(halfpixel.x, -halfpixel.y) * offset); sum += t * 2.0;\n" +
            "    t = texture2D(inTexture, uv + vec2(0.0, -halfpixel.y * 2.0) * offset); sum += t;\n" +
    ...
    private final String kawaseDown = "#version 120\n" +
            ...
            "    vec4 t;\n" +
            "    t = texture2D(inTexture, uv - halfpixel.xy * offset); sum += t;\n" +
            "    t = texture2D(inTexture, uv + halfpixel.xy * offset); sum += t;\n" +
            "    t = texture2D(inTexture, uv + vec2(halfpixel.x, -halfpixel.y) * offset); sum += t;\n" +
            "    t = texture2D(inTexture, uv - vec2(halfpixel.x, -halfpixel.y) * offset); sum += t;\n" +
    ...
    private final String roundedRect = "#version 120\n" +
                    ...
                    "    vec2 p = rectHalf - (uv * rectSize);\n" +
                    "    vec2 q = rectHalf - radius - 1.0;\n" +
                    "\n" +
                    "    float dist = roundSDF(p, q, radius);\n" +
                    "    float smoothedAlpha = (1.0 - smoothstep(0.0, 1.0, dist)) * color.a;\n" +
                    "\n" +
                    "    if (smoothedAlpha < 0.01) discard;\n" +
                    "\n" +
    ...
    public ShaderUtils(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "neo:shaders/vertex.vsh");
    }
